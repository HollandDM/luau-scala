...
- **Off-Driver completions enqueue, never resume inline.** An **Async primitive** may complete on any thread (an async-IO pool, a JS microtask); it posts a resume onto the **Run queue** instead of calling `lua_resume` directly.
