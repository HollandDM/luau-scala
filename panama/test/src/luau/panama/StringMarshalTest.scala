...
class StringMarshalTest extends munit.FunSuite:
  ...
  test("ASCII string round-trip via push/read".ignore) {
    PanamaState.use { ps =>
  ...
  test("UTF-8 multibyte string round-trip (Japanese)".ignore) {
    PanamaState.use { ps =>
      val s = "こんにちは"
  ...
  test("toBytes returns None for non-string stack slot".ignore) {
    PanamaState.use { ps =>
  ...
  test("empty string round-trip".ignore) {
    PanamaState.use { ps =>
      ps.pushString(ps.L, "")
